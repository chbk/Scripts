...
echo "refine_comptr_output... line 104" >> debug_time.log
time (awk '$3=="exon"' $annot | awk -f $MAKEOK | awk -f $GFF2GFF | sort -k12,12 -k4,4n -k5,5n > annot_exons.gff) &>> debug_time.log
time (awk '$3=="gene"' $annot | awk -f $MAKEOK > annot_genes.gff) &>> debug_time.log
time (awk -v toadd=transcript -v fldno=12 -f $BOUNDARIES annot_exons.gff | awk -v fileRef=annot_exons.gff 'BEGIN{while (getline < fileRef >0){gnid[$12]=$10}} {print $1, $2, $3, $4, $5, $6, $7, $8, "gene_id", gnid[$10], $9, $10}' | awk -f $GFF2GFF > annot_tr.gff) &>> debug_time.log
echo "refine_comptr_output... line 108" >> debug_time.log
...
echo "refine_comptr_output... line 115" >> debug_time.log
time ($MAKESUM $mytr > prediction_sumstat.txt) &>> debug_time.log
echo done >&2
...
echo "refine_comptr_output... line 122" >> debug_time.log
time (awk '$3=="exon"' $mytr | awk -f $MAKEOK > $mytrbase\_exons.gff) &>> debug_time.log
time (awk '{nbex[$12]++}END{for(t in nbex){print t, nbex[t];}}' $mytrbase\_exons.gff > trid_nbex.txt) &>> debug_time.log
echo done >&2

# 4. I am running comptr on the input transcripts and the annotation
####################################################################
echo I am running comptr >&2
echo "refine_comptr_output... line 130" >> debug_time.log
time ($COMPTR $mytrbase\_exons.gff annot_exons.gff -s 5000 -o $mytrbase\_vs_annot.tsv) &>> debug_time.log
echo The classes from comptr >&2
echo "refine_comptr_output... line 133" >> debug_time.log
time (awk '{print $2}' $mytrbase\_vs_annot.tsv | sort | uniq -c | sort -k1,1nr >&2) &>> debug_time.log
echo done >&2
...
echo "refine_comptr_output... line 150" >> debug_time.log
time (awk '$2=="Monoexonic"||$2=="Overlap"||$2=="Intergenic_or_antisense"{split($1,a,"\""); print a[2]}' $mytrbase\_vs_annot.tsv > monoex_overlap_igas_trids.txt) &>> debug_time.log
# TCONS_00000001
# 40358 (1 fields)
...
echo "refine_comptr_output... line 158" >> debug_time.log
time (awk -v fileRef=monoex_overlap_igas_trids.txt 'BEGIN{while (getline < fileRef >0){ok["\""$1"\";"]=1}} (($3=="transcript")&&(ok[$12]==1))' $mytrbase\_complete.gff > $mytrbase\_tr_monoex_overlap_igas.gff) &>> debug_time.log
...
echo "refine_comptr_output... line 163" >> debug_time.log
time ($OVERLAP $mytrbase\_tr_monoex_overlap_igas.gff annot_genes.gff -st 1 -f gn -o $mytrbase\_tr_monoex_overlap_igas_strover_genes.gff) &>> debug_time.log
...
echo "refine_comptr_output... line 168" >> debug_time.log
time ($OVERLAP $mytrbase\_tr_monoex_overlap_igas_strover_genes.gff annot_genes.gff -f gn -o $mytrbase\_tr_monoex_overlap_igas_strover_genes_over_genes.gff) &>> debug_time.log
...
echo "refine_comptr_output... line 174" >> debug_time.log
time (awk '$(NF-2)==1' $mytrbase\_tr_monoex_overlap_igas_strover_genes_over_genes.gff > $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes.gff) &>> debug_time.log
...
echo "refine_comptr_output... line 182" >> debug_time.log
time ($OVERLAP $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes.gff annot_genes.gff -st 1 -i 1 -f gn -o $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes_totstrincl.gff) &>> debug_time.log
...
# the included ones
echo "refine_comptr_output... line 189" >> debug_time.log
time (awk '$NF==1{print $12}' $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes_totstrincl.gff > included_but_not_respecting_annot_tr_structure.txt) &>> debug_time.log
# "TCONS_00005235";
# 9446 (1 fields)
# the extension ones
time (awk '$NF==0{print $12}' $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes_totstrincl.gff > extension_but_not_respecting_annot_tr_structure.txt) &>> debug_time.log
# "TCONS_00005247";
# 17926 (1 fields)

# the ones that overlap the annotated genes on the opposite strand
time (awk '(($NF==1)&&($(NF-2)==0)){print $12}' $mytrbase\_tr_monoex_overlap_igas_strover_genes_over_genes.gff > as_overlapping.txt) &>> debug_time.log
# "TCONS_00000001";
...
# 140 (16 fields)
echo "refine_comptr_output... line 211" >> debug_time.log
time (awk -v ext=500 'BEGIN{OFS="\t"}(($NF==0)&&($(NF-2)==0)){if($7=="+"){pos=$4}else{pos=$5} if(pos-ext>0){print $1, ".", ".", pos-ext+1, pos+ext, ".", $7, ".", "transcript_id", $12}}' $mytrbase\_tr_monoex_overlap_igas_strover_genes_over_genes.gff | awk -f $GFF2GFF > $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext.gff) &>> debug_time.log
time ($OVERLAP $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext.gff $annbase\_capped_sites_nr.gff -st -1 -f cap -o $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext_overanntss.gff) &>> debug_time.log
time (awk '$NF!=0{split($10,a,"\""); print a[2]}' $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext_overanntss.gff | sort | uniq > antisense_500bpwaytssannot.txt) &>> debug_time.log
...
#######################################################################
echo "refine_comptr_output... line 250" >> debug_time.log
time (awk -v fldgn=10 -v fldtr=12 -f $MAKEINTRONS annot_exons.gff | awk '{split($10,a,"\""); print $1":"$4":"$5":"$7, a[2]}' | sort | uniq | awk '{gnlist[$1]=(gnlist[$1])($2)(",")}END{for(i in gnlist){print i, gnlist[i]}}' > annot_introns_coord_gnlist.tsv) &>> debug_time.log
# 2:140209598:140210486:+ ENSSSCG00000014326,
# 217796 (2 fields)
# 6.b. make the introns from the predicted transcripts
#######################################################
time (sort -k12,12 -k4,4n -k5,5n $mytrbase\_exons.gff | awk -v fldgn=10 -v fldtr=12 -f $MAKEINTRONS > $mytrbase\_introns.gff) &>> debug_time.log
...
############################################################################################################################################
echo "refine_comptr_output... line 261" >> debug_time.log
time (awk -v fileRef=annot_introns_coord_gnlist.tsv 'BEGIN{OFS="\t"; while (getline < fileRef >0){gnlist[$1]=$2}} {if(gnlist[$1":"$4":"$5":"$7]!=""){split($12,a,"\""); split(gnlist[$1":"$4":"$5":"$7],b,","); k=1; while(b[k]!=""){ok[a[2],b[k]]++; if(ok[a[2],b[k]]==1){nbgn[a[2]]++; gn[a[2],nbgn[a[2]]]=b[k];} k++}}} END{for(t in nbgn){for(i=1; i<=nbgn[t]; i++){s[t]=s[t](gn[t,i])(",")} print t, s[t]}}' $mytrbase\_introns.gff > $mytrbase\_transcriptid_gnlistwithcommonintron.tsv) &>> debug_time.log
# TCONS_00024222	ENSSSCG00000028855,
# 61261 (2 fields)
# 6.d. make the predicted monoexonic transcripts in bed format and with transcript id
#####################################################################################
echo "refine_comptr_output... line 267" >> debug_time.log
time (awk '$3=="exon"{nbex[$12]++; ex[$12,nbex[$12]]=$0} END{for(t in nbex){if(nbex[t]==1){print ex[t,1]}}}' $mytrbase\_complete.gff | awk -v fld=12 -f $GTF2BED > $mytrbase\_monoextr_exons.bed) &>> debug_time.log
# X	50836285	50838528	TCONS_00119421	0	-
# 9519 (6 fields)
# 6.e. make the reference mononoexonic transcripts in bed format and with transcript id
#######################################################################################
echo "refine_comptr_output... line 273" >> debug_time.log
time (awk '{nbex[$12]++; ex[$12,nbex[$12]]=$0} END{for(t in nbex){if(nbex[t]==1){print ex[t,1]}}}' annot_exons.gff | awk -v fld=12 -f $GTF2BED > annot_monoextr_exons.bed) &>> debug_time.log
...
##############################################################################
echo "refine_comptr_output... line 281" >> debug_time.log
time ($INTER -a $mytrbase\_monoextr_exons.bed -b annot_monoextr_exons.bed -s -wao | awk 'BEGIN{OFS="\t"} {ok[$4]=1; if($NF!=0){if($2==$8&&$3==$9){knownref[$4]=(knownref[$4])($10)(",")}else{if($8<$2||$9>$3){extref[$4]=(extref[$4])($10)(",")}}}} END{for(t in ok){if(knownref[t]!=""){print t, "known", knownref[t]}else{if(extref[t]!=""){print t, "extension", extref[t]}else{print t, "novel", "."}}}}' > $mytrbase\_monoextr_id_class_reftrlist.tsv) &>> debug_time.log
...
#####################################################
echo "refine_comptr_output... line 291" >> debug_time.log
time (awk -v fileRef1=$mytrbase\_complete.gff -v fileRef2=annot_tr.gff -v fileRef3=$mytrbase\_transcriptid_gnlistwithcommonintron.tsv -v fileRef4=$mytrbase\_monoextr_id_class_reftrlist.tsv -f $ADDCLASS $mytrbase\_comp_refinedclass_nbex.tsv > $mytrbase\_comp_refinedclass_nbex_intermclass.tsv) &>> debug_time.log
...
echo "refine_comptr_output... line 304" >> debug_time.log
time (rm annot_exons.gff annot_genes.gff annot_tr.gff) &>> debug_time.log
time (rm $mytrbase\_exons.gff $mytrbase\_introns.gff) &>> debug_time.log
time (rm trid_nbex.txt) &>> debug_time.log
time (rm $mytrbase\_vs_annot.tsv) &>> debug_time.log
time (rm monoex_overlap_igas_trids.txt) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas.gff) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_strover_genes.gff) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_strover_genes_over_genes.gff) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes.gff) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_strover_genes_ok_over_genes_totstrincl.gff) &>> debug_time.log
time (rm included_but_not_respecting_annot_tr_structure.txt) &>> debug_time.log
time (rm extension_but_not_respecting_annot_tr_structure.txt) &>> debug_time.log
time (rm as_overlapping.txt) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext.gff) &>> debug_time.log
time (rm $mytrbase\_tr_monoex_overlap_igas_over_genes_ko_tss_500bpext_overanntss.gff) &>> debug_time.log
time (rm antisense_500bpwaytssannot.txt) &>> debug_time.log
time (rm annot_introns_coord_gnlist.tsv) &>> debug_time.log
time (rm $mytrbase\_transcriptid_gnlistwithcommonintron.tsv) &>> debug_time.log
time (rm $mytrbase\_monoextr_exons.bed annot_monoextr_exons.bed $mytrbase\_monoextr_id_class_reftrlist.tsv) &>> debug_time.log
time (rm $mytrbase\_comp_refinedclass_nbex.tsv) &>> debug_time.log
echo done >&2
